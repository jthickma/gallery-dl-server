...
| `‑‑cors‑allow‑origins` | `CORS_ALLOW_ORIGINS` | &cross;     | `str`  | `*`<br>`http://localhost:3000,https://example.com`                 | `*`           | Set allowed CORS origins           |
...
- Archive creation uses temporary disk space equal to the ZIP size.
...
```javascript
javascript: (function () {
  var url = 'http://${host}:${port}/gallery-dl/q',
